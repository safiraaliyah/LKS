...
<div class="max-w-4xl mx-auto mt-10 bg-white p-6 rounded-lg shadow-lg">
  ...
  <form id="editProfileForm" action="/update-lks" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf

    <input type="hidden" name="id" value="{{ $lks->id }}">

    <img src="../img/lks/{{ $lks->foto_lks }}" alt="" class="rounded-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      ...
      <div class="md:col-span-2">
        <input type="text" name="nama_lks" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ $lks->nama_lks }}">
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      <label class="md:text-left pr-4 text-gray-700">Ketua LKS</label>
      <div class="md:col-span-2">
        <input type="text" name="ketua_lks" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ $lks->ketua_lks }}">
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      <label class="md:text-left pr-4 text-gray-700">Alamat LKS</label>
      <div class="md:col-span-2">
        <textarea name="alamat_lks" class="w-full p-2 border border-gray-300 rounded-lg" rows="1">{{ $lks->alamat_lks }}</textarea>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      <label class="md:text-left pr-2 text-gray-700">Nomor & Tanggal Akte Notaris</label>
      <div class="md:col-span-2 space-y-2">
        <input type="text" name="nomor_notaris" placeholder="Nomor Notaris" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ $lks->nomor_notaris }}">
        <input type="datetime-local" name="tanggal_akte_notaris" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ $lks->tanggal_akte_notaris }}">
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      <label class="md:text-left pr-4 text-gray-700">Nomor & Tanggal Tanda Daftar</label>
      <div class="md:col-span-2 space-y-2">
        <input type="datetime-local" name="kontrak_awal" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ $lks->kontrak_awal }}">
        <input type="datetime-local" name="kontrak_akhir" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ $lks->kontrak_akhir }}">
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      <label class="md:text-left pr-4 text-gray-700">Kontak Pengurus</label>
      <div class="md:col-span-2">
        <input type="text" name="kontak_pengurus" class="w-full p-2 border border-gray-300 rounded-lg" value="{{ $lks->kontak_pengurus }}">
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      ...
      <div class="md:col-span-2">
        <select name="akreditasi_lks" class="w-full p-2 border border-gray-300 rounded-lg">
          <option value="A" {{ $lks->akreditasi_lks == 'A' ? 'selected' : '' }}>A</option>
          <option value="B" {{ $lks->akreditasi_lks == 'B' ? 'selected' : '' }}>B</option>
          <option value="C" {{ $lks->akreditasi_lks == 'C' ? 'selected' : '' }}>C</option>
        </select>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      ...
      <div class="md:col-span-2">
        <select name="jenis_lks" class="w-full p-2 border border-gray-300 rounded-lg">
          <option value="LKS Kota" {{ $lks->jenis_lks == 'LKS Kota' ? 'selected' : '' }}>LKS Kota</option>
          <option value="LKS Provinsi" {{ $lks->jenis_lks == 'LKS Provinsi' ? 'selected' : '' }}>LKS Provinsi</option>
          <option value="LKS Nasional" {{ $lks->jenis_lks == 'LKS Nasional' ? 'selected' : '' }}>LKS Nasional</option>
        </select>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
      ...
      <div class="md:col-span-2">
        <select name="jenis_pelayanan" class="w-full p-2 border border-gray-300 rounded-lg">
          <option value="Anak" {{ $lks->jenis_pelayanan == 'Anak' ? 'selected' : '' }}>Anak</option>
          <option value="Disabilitas" {{ $lks->jenis_pelayanan == 'Disabilitas' ? 'selected' : '' }}>Disabilitas</option>
          <option value="Lanjut Usia" {{ $lks->jenis_pelayanan == 'Lanjut Usia' ? 'selected' : '' }}>Lanjut Usia</option>
        </select>
      </div>
    </div>
