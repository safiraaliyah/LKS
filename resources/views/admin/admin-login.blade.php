...
<body class="bg-cover bg-center bg-no-repeat" style="background-image: url('{{ asset('bg_login.jpg') }}');">
    <div class="flex items-center justify-center h-screen">
        <div class="bg-white bg-opacity-90 p-8 rounded-lg shadow-lg max-w-md w-full mx-4 md:mx-auto">
            <div class="flex justify-center mb-4">
                <img src="{{ asset('logo_kota.png') }}" alt="Logo" class="w-24 h-24">
            </div>    
            <h1 class="text-3xl font-bold text-center mb-6">Log In Admin</h1>
            <form class="space-y-4" method="POST" action="{{ route('admin.admin-login') }}">
                @csrf
                <div>
